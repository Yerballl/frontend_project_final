...
// Обновленная функция для форматирования валюты
const formatCurrency = (amountStr, currencyCode = 'RUB') => { // Принимает currencyCode, по умолчанию RUB
    const amount = parseFloat(amountStr);
    if (isNaN(amount)) {
        // Возвращаем 0.00 с указанием валюты или просто прочерк, если сумма невалидна
    ...
    return new Intl.NumberFormat('ru-RU', { // Используем 'ru-RU' для единообразия, но валюта будет из currencyCode
        style: 'currency',
        currency: currencyCode,
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
...
function CategoryList({ categories, onEdit, onDelete, isLoading, error, currency }) {
    if (isLoading) {
        return <div className="animate-pulse text-center py-4">Загрузка категорий...</div>;
    }
                ...
                >
                    <div>
                        <div className="flex items-center mb-2">
                            <div
                                className="w-10 h-10 rounded-full flex items-center justify-center text-white text-xl mr-3 flex-shrink-0"
                                style={{ backgroundColor: category.color || '#6366f1' }} // Используем цвет категории или дефолтный
                            >
                                {/* Отображаем иконку категории, если есть, иначе первую букву имени или дефолтную иконку */}
                                <span>{category.icon || (category.name ? category.name.charAt(0).toUpperCase() : '💰')}</span>
                            </div>
                            <span className="font-semibold text-gray-800 truncate" title={category.name}>
                                {category.name}
                            </span>
                        ...
                        <div className="text-right mb-3 pr-1">
                            <span
                                className={`font-bold text-xl ${parseFloat(category.balance) >= 0 ? 'text-green-600' : 'text-red-600'}`}
                            >
                                {/* Передаем текущую валюту в formatCurrency */}
