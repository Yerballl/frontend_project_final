...
import React from 'react';
// Убираем useSelector, так как данные о категории и счете лучше передавать через пропсы

// Компонент теперь принимает 'currency' и детали категории/счета через пропсы
const TransactionItem = ({
  transaction,
  onEdit,
  onDelete,
  currency = 'RUB', // Валюта по умолчанию
  categoryName,     // Имя категории (передается из TransactionList)
  categoryColor,    // Цвет категории (передается из TransactionList)
  categoryIcon,     // Иконка категории (передается из TransactionList)
  accountName,      // Имя счета (передается из TransactionList)
  accountIcon,      // Иконка счета (передается из TransactionList)
}) => {
  const amount = parseFloat(transaction.amount) || 0;
  const isIncome = transaction.type === 'income';
  ...
  if (transaction.transaction_date) {
    try {
      const datePart = transaction.transaction_date.split('T')[0];
      // Убедимся, что дата парсится как локальная, а не UTC, чтобы избежать смещения на день
  ...
  return (
      <div className={`flex items-center justify-between p-3 mb-2 rounded-lg shadow-sm transition-all hover:shadow-md ${isIncome ? 'bg-green-50 hover:bg-green-100' : 'bg-red-50 hover:bg-red-100'}`}>
        <div className="flex items-center overflow-hidden">
          {/* Отображаем иконку и цвет категории, переданные через пропсы */}
          <div
              className="w-8 h-8 rounded-full flex items-center justify-center text-sm text-white mr-3 flex-shrink-0"
              style={{ backgroundColor: categoryColor || (isIncome ? '#10B981' : '#EF4444') }} // Дефолтные цвета, если не передан
              title={categoryName || 'Без категории'}
          >
            {categoryIcon || (isIncome ? '➕' : '➖')} {/* Дефолтные иконки */}
          </div>

          <div className="overflow-hidden">
            <div className="font-medium text-gray-800 text-sm">
              {/* Отображаем имя категории, переданное через пропсы */}
              {categoryName || (transaction.category_id ? `ID: ${transaction.category_id}` : 'Без категории')}
            </div>
            {accountName && ( // Отображаем имя и иконку счета, переданные через пропсы
                <div className="text-xs text-gray-500">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
          <div className={`font-semibold text-sm ${isIncome ? 'text-green-700' : 'text-red-700'}`}>
            {isIncome ? '+' : '-'}
            {formattedAmount} {/* Отображаем отформатированную сумму */}
          </div>
          <button
              onClick={onEdit} // onEdit теперь должен вызываться из TransactionList с объектом транзакции
              className="text-gray-400 hover:text-indigo-600 p-1 rounded-full hover:bg-indigo-50 transition"
              title="Редактировать"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button
              onClick={onDelete} // onDelete теперь должен вызываться из TransactionList с ID транзакции
              className="text-gray-400 hover:text-red-600 p-1 rounded-full hover:bg-red-50 transition"
              title="Удалить"
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
